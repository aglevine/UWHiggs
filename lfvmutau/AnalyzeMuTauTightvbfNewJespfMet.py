...
def correctJesMet(metJes,metType1,metRaw,metJesPhi,metPhiType1,metPhiRaw,Objectpx,Objectpy,ObjectEt):

        metpx = metJes*math.cos(metJesPhi)+metRaw*math.cos(metPhiRaw)-metType1*math.cos(metPhiType1)
        metpy = metJes*math.cos(metJesPhi)+metRaw*math.cos(metPhiRaw)-metType1*math.cos(metPhiType1)
        met = sqrt(metpx*metpx+metpy*metpy)


        full_pt=met+ObjectEt
        full_x=metpx+Objectpx
        full_y=metpy+Objectpy
...
def collMass(row,pfMetEt,pfMetPhi):
        taupx = row.tPt*math.cos(row.tPhi)
        taupy = row.tPt*math.sin(row.tPhi)
        taupt=  row.tPt

        metpx = pfMetEt*math.cos(pfMetPhi)
        metpy = pfMetEt*math.sin(pfMetPhi)
        met = pfMetEt
...
class AnalyzeMuTauTightvbfNewJespfMet(MegaBase):
    if treeSkim == True:
	tree = 'New_Tree'
    else:
    	tree = 'mt/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeMuTauTightvbfNewJespfMet, self).__init__(tree, outfile, **kwargs)
...
#	names = ["gtcmp"] names used to compare global tags (no cuts)
	...
	for x in range(0,namesize):
	    ...
	    self.book(names[x],"collMass_jes_plus","collMass_jes_plus",500,0,500);
...
### vbf ###
	...
	histos[name+'/collMass'].Fill(collMass(row,row.pfMetEt,row.pfMetPhi),weight)
	histos[name+'/collMass_jes_plus'].Fill(collMass(row,row.pfMet_jes_plus_Et,row.pfMet_jes_plus_Phi),weight)
        histos[name+'/collMass_jes_minus'].Fill(collMass(row,row.pfMet_jes_minus_Et,row.pfMet_jes_minus_Phi),weight)
	...
	if jetcorr =="none":
		return row.tMtToPFMET
	elif jetcorr=="jesplus":
		Taupx = row.tPt*math.cos(row.tPhi)
        	Taupy = row.tPt*math.sin(row.tPhi)
        	Taupz = row.tPt*math.sinh(row.tEta)
        	Taup  = math.sqrt(Taupx*Taupx+Taupy*Taupy+Taupz*Taupz)
        	TauMass = row.tMass
        	TauE    = math.sqrt(TauMass*TauMass+Taup*Taup)
		Tau = ROOT.TLorentzVector(Taupx,Taupy,Taupz,TauE)
		TauEt = Tau.Et()
                jestMtToPFMET = correctJesMet(row.tMtToPfMet_jes_plus,row.tMtToPfMet_Ty1,row.tMtToPFMET,row.pfMet_jes_plus_Phi,row.type1_pfMetPhi,row.pfMetPhi,Taupx,Taupy,TauEt)

		return jestMtToPFMET
	elif jetcorr=="jesminus":
                Taupx = row.tPt*math.cos(row.tPhi)
                Taupy = row.tPt*math.sin(row.tPhi)
                Taupz = row.tPt*math.sinh(row.tEta)
                ...
                Tau = ROOT.TLorentzVector(Taupx,Taupy,Taupz,TauE)
                TauEt = Tau.Et()
                jestMtToPFMET = correctJesMet(row.tMtToPfMet_jes_minus,row.tMtToPfMet_Ty1,row.tMtToPFMET,row.pfMet_jes_minus_Phi,row.type1_pfMetPhi,row.pfMetPhi,Taupx,Taupy,TauEt)
                return jestMtToPFMET
    def getmPhiMETPhiType1(self,row):
	if jetcorr =="none":
                return deltaPhi(row.mPhi,row.pfMetPhi)
        elif jetcorr=="jesplus":
		pfMetJesPlusPhi = row.pfMet_jes_plus_Phi - row.type1_pfMetPhi+row.pfMetPhi
                return deltaPhi(row.mPhi,pfMetJesPlusPhi)
        elif jetcorr=="jesminus":
		pfMetJesMinusPhi = row.pfMet_jes_minus_Phi - row.type1_pfMetPhi+row.pfMetPhi
                return deltaPhi(row.mPhi,pfMetJesMinusPhi)
    def gettPhiMETPhiType1(self,row):
        if jetcorr =="none":
                return deltaPhi(row.tPhi,row.pfMetPhi)
        elif jetcorr=="jesplus":
    ...
    def getmMtToPfMet(self,row):
        if jetcorr =="none":
                return row.mMtToPFMET
        elif jetcorr=="jesplus":
                Mupx = row.mPt*math.cos(row.mPhi)
                Mupy = row.mPt*math.sin(row.mPhi)
                Mupz = row.mPt*math.sinh(row.mEta)
                Mup  = math.sqrt(Mupx*Mupx+Mupy*Mupy+Mupz*Mupz)
		MuMass = .1057
                MuE    = math.sqrt(MuMass*MuMass+Mup*Mup)
                Mu = ROOT.TLorentzVector(Mupx,Mupy,Mupz,MuE)
                MuEt = Mu.Et()
                jesmMtToPFMET = correctJesMet(row.mMtToPfMet_jes_plus,row.mMtToPfMet_Ty1,row.mMtToPFMET,row.pfMet_jes_plus_Phi,row.type1_pfMetPhi,row.pfMetPhi,Mupx,Mupy,MuEt)
                return jesmMtToPFMET
        elif jetcorr=="jesminus":
                Mupx = row.mPt*math.cos(row.mPhi)
                Mupy = row.mPt*math.sin(row.mPhi)
                Mupz = row.mPt*math.sinh(row.mEta)
		...
		MuMass = .1057
                ...
                Mu = ROOT.TLorentzVector(Mupx,Mupy,Mupz,MuE)
                MuEt = Mu.Et()
                jesmMtToPFMET = correctJesMet(row.mMtToPfMet_jes_minus,row.mMtToPfMet_Ty1,row.mMtToPFMET,row.pfMet_jes_minus_Phi,row.type1_pfMetPhi,row.pfMetPhi,Mupx,Mupy,MuEt)
                return jesmMtToPFMET
...
#          return False
    ...
    def process(self):
	    ...
	    #self.fill_histos(row,'gtcmp')	#only when applying no cuts
	    if Isiso == False:
		obj1iso = True
		obj2iso = True
		obj2isogg = True
